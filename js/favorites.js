...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function applyMobileStyles() {
        ...
        if (window.innerWidth <= 576) {
            // التأكد من أن طريقة العرض هي grid
            favoritesContainer.classList.remove('list-view');
            favoritesContainer.classList.add('grid-view');

            // تحديث أزرار طريقة العرض
            viewTabs.forEach(tab => {
                tab.classList.remove('active');
                if (tab.getAttribute('data-view') === 'grid') {
    ...
    // إضافة مستمع حدث لتغيير حجم الشاشة
    window.addEventListener('resize', applyMobileStyles);

    // تحديث عدد العناصر في السلة عند تحميل الصفحة
    updateCartCount();

    // تطبيق تنسيقات الشاشات الصغيرة عند تحميل الصفحة
    applyMobileStyles();
});