...
const productsData = [
    ...
    function initPage() {
        try {
            ...
            setTimeout(() => {
                ...
                loadPharmacyOffers();
                loadHypermarketOffers();
                loadRestaurantOffers();
                loadCarServiceOffers();
                loadFeaturedOffers(); // تحميل عروض مميزة بدلاً من عشوائية
            ...
            showToast(errorToast, errorToastMessage, "حدث خطأ أثناء تحميل المنتجات.");
        }
    }
    ...
    function updateCartInfo() {
        try {
            ...
            console.error("حدث خطأ أثناء تحديث معلومات السلة:", error);
        }
    }
    ...
    function updateFavoriteButtonsState() {
        try {
            ...
            console.error("حدث خطأ أثناء تحديث حالة أزرار المفضلة:", error);
        }
    }
    ...
    function loadProductsByService(service, container) {
        try {
            ...
            console.error("حدث خطأ أثناء تحميل المنتجات حسب الخدمة:", error);
        }
    }
    ...
    function createProductCard(product) {
        try {
            ...
            console.error("حدث خطأ أثناء إنشاء كرت المنتج:", error);
            return null;
        }
    }
    ...
    function generateRatingStars(rating) {
        try {
            ...
            console.error("حدث خطأ أثناء إنشاء نجوم التقييم:", error);
            return '';
        }
    }
    ...
    function setupPagination() {
        try {
            ...
            console.error("حدث خطأ أثناء تهيئة الترقيم:", error);
        }
    }
    ...
    function updatePaginationState() {
        try {
            ...
            console.error("حدث خطأ أثناء تحديث حالة أزرار الترقيم:", error);
        }
    }
    ...
    function animateProducts() {
        try {
            ...
            console.error("حدث خطأ أثناء إضافة تأثير انتقالي للمنتجات:", error);
        }
    }
    ...
    function showToast(toastElement, messageElement, message) {
        try {
            ...
            console.error("حدث خطأ أثناء عرض الإشعارات:", error);
        }
    }
    ...
    function setupEventListeners() {
        try {
            ...
            // مستمع حدث لزر تحديث العروض المميزة
            ...
            if (refreshRandomOffersBtn) {
                refreshRandomOffersBtn.addEventListener('click', function() {
                    ...
                    setTimeout(() => {
                        ...
                        // عرض رسالة نجاح
                        showToast(successToast, successToastMessage, "تم تحديث العروض المميزة بنجاح");

                        // إضافة تأثير انتقالي للمنتجات
                        const randomOfferCards = document.querySelectorAll("#randomOffers .product-card");
            ...
            if (backToTopBtn) {
                ...
                // مستمع حدث للنقر على زر العودة للأعلى
            ...
            function createQuickViewModal(product) {
                try {
                    ...
                    console.error("حدث خطأ أثناء إنشاء نافذة منبثقة للعرض السريع:", error);
                }
            }
        } catch (error) {
            console.error("حدث خطأ أثناء تهيئة مستمع الأحداث:", error);
        }
    }
    ...
    function updateCategoryOptions(service) {
        try {
            ...
            console.error("حدث خطأ أثناء تحديث خيارات الفئات:", error);
        }
    }
    ...
    function updateActiveFilters() {
        try {
            ...
            console.error("حدث خطأ أثناء تحديث الفلاتر النشطة:", error);
        }
    }
    ...
    function addActiveFilter(type, value) {
        try {
            ...
            console.error("حدث خطأ أثناء إضافة فلتر نشط:", error);
        }
    }
    ...
    function removeFilter(type) {
        try {
            ...
            console.error("حدث خطأ أثناء إزالة الفلتر:", error);
        }
    }
        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (filteredProducts.length === 0) {
                    ...
                    // إخفاء أقسام المنتجات إذا لم تكن هناك منتجات
                    ...
                    if (noProductsMessage) noProductsMessage.style.display = "none";
                    // إظهار أقسام المنتجات إذا كانت هناك منتجات
                    const sections = document.querySelectorAll('.product-category-section');
                    sections.forEach(section => {
            ...
            console.error("حدث خطأ أثناء تطبيق الفلاتر:", error);
        }
    }
    ...
    function displayFilteredProducts() {
        try {
            // الحد الأقصى للمنتجات المعروضة في كل صفحة
            const maxProductsPerPage = productsPerPage;

            // الحصول على المنتجات المراد عرضها في الصفحة الحالية
            const productsToDisplay = filteredProducts.slice((currentPage - 1) * maxProductsPerPage, currentPage * maxProductsPerPage);

            // تفريغ الحاويات
            if (pharmacyProducts) pharmacyProducts.innerHTML = '';
            if (hypermarketProducts) hypermarketProducts.innerHTML = '';
            if (restaurantProducts) restaurantProducts.innerHTML = '';
            if (carServiceProducts) carServiceProducts.innerHTML = '';

            // إضافة المنتجات إلى الحاويات المناسبة
            productsToDisplay.forEach(product => {
                const container = getProductContainer(product.service);
                if (container) {
                    container.appendChild(createProductCard(product));
                }
            });
        } catch (error) {
            console.error("حدث خطأ أثناء عرض المنتجات المصفاة:", error);
        }
    }

    // الحصول على الحاوية المناسبة للمنتج
    function getProductContainer(service) {
        // التحقق من وجود الحاويات قبل الإرجاع
        switch(service) {
            case 'pharmacy': return pharmacyProducts || null;
            case 'hypermarket': return hypermarketProducts || null;
            case 'restaurant': return restaurantProducts || null;
            case 'car-service': return carServiceProducts || null;
            default: return null;
        }
    }

    // تحميل عروض الصيدليات
    function loadPharmacyOffers() {
        try {
            // تصفية المنتجات لعرض عروض الصيدليات فقط
            const offers = productsData.filter(product => product.service === "pharmacy" && product.discount > 0);

            // التأكد من وجود على الأقل 3 منتجات
            let displayOffers = [...offers];
            if (offers.length < 3) {
                // إضافة منتجات إضافية من نفس الفئة للوصول إلى 3 منتجات
                const additionalProducts = productsData.filter(product => product.service === "pharmacy" && !offers.some(o => o.id === product.id));
                displayOffers = [...offers, ...additionalProducts.slice(0, 3 - offers.length)];
            }
            ...
            if (pharmacyOffers) {
                ...
                // إضافة المنتجات إلى الحاوية
                displayOffers.forEach(product => {
                    pharmacyOffers.appendChild(createProductCard(product));
                });
            }
        } catch (error) {
            console.error("حدث خطأ أثناء تحميل عروض الصيدليات:", error);
        }
    }

    // تحميل عروض الهايبر ماركت
    function loadHypermarketOffers() {
        try {
            // تصفية المنتجات لعرض عروض الهايبر ماركت فقط
            const offers = productsData.filter(product => product.service === "hypermarket" && product.discount > 0);

            // التأكد من وجود على الأقل 3 منتجات
            let displayOffers = [...offers];
            if (offers.length < 3) {
                // إضافة منتجات إضافية من نفس الفئة للوصول إلى 3 منتجات
                const additionalProducts = productsData.filter(product => product.service === "hypermarket" && !offers.some(o => o.id === product.id));
                displayOffers = [...offers, ...additionalProducts.slice(0, 3 - offers.length)];
            }
            ...
            if (hypermarketOffers) {
                ...
                // إضافة المنتجات إلى الحاوية
                displayOffers.forEach(product => {
                    hypermarketOffers.appendChild(createProductCard(product));
                });
            }
        } catch (error) {
            console.error("حدث خطأ أثناء تحميل عروض الهايبر ماركت:", error);
        }
    }

    // تحميل عروض المطاعم
    function loadRestaurantOffers() {
        try {
            // تصفية المنتجات لعرض عروض المطاعم فقط
            const offers = productsData.filter(product => product.service === "restaurant" && product.discount > 0);

            // التأكد من وجود على الأقل 3 منتجات
            let displayOffers = [...offers];
            if (offers.length < 3) {
                // إضافة منتجات إضافية من نفس الفئة للوصول إلى 3 منتجات
                const additionalProducts = productsData.filter(product => product.service === "restaurant" && !offers.some(o => o.id === product.id));
                displayOffers = [...offers, ...additionalProducts.slice(0, 3 - offers.length)];
            }
            ...
            if (restaurantOffers) {
                ...
                // إضافة المنتجات إلى الحاوية
                displayOffers.forEach(product => {
                    restaurantOffers.appendChild(createProductCard(product));
                });
            }
        } catch (error) {
            console.error("حدث خطأ أثناء تحميل عروض المطاعم:", error);
        }
    }

    // تحميل عروض خدمات السيارات
    function loadCarServiceOffers() {
        try {
            // تصفية المنتجات لعرض عروض خدمات السيارات فقط
            const offers = productsData.filter(product => product.service === "car-service" && product.discount > 0);

            // التأكد من وجود على الأقل 3 منتجات
            let displayOffers = [...offers];
            if (offers.length < 3) {
                // إضافة منتجات إضافية من نفس الفئة للوصول إلى 3 منتجات
                const additionalProducts = productsData.filter(product => product.service === "car-service" && !offers.some(o => o.id === product.id));
                displayOffers = [...offers, ...additionalProducts.slice(0, 3 - offers.length)];
            }
            ...
            if (carServiceOffers) {
                ...
                // إضافة المنتجات إلى الحاوية
                displayOffers.forEach(product => {
                    carServiceOffers.appendChild(createProductCard(product));
                });
            }
        } catch (error) {
            console.error("حدث خطأ أثناء تحميل عروض خدمات السيارات:", error);
        }
    }

    // تحميل عروض مميزة بدلاً من عشوائية
    function loadFeaturedOffers() {
        try {
            // تصفية المنتجات لعرض العروض المميزة فقط
            const featuredOffers = productsData.filter(product =>
                product.discount > 10 || // عروض بخصم أكثر من 10%
                (product.tags && product.tags.includes('bestseller')) || // المنتجات الأكثر مبيعاً
                (product.tags && product.tags.includes('featured')) // المنتجات المميزة
            );

            // التأكد من وجود على الأقل 3 منتجات
            let displayOffers = [...featuredOffers];
            if (featuredOffers.length < 3) {
                // إضافة منتجات إضافية للوصول إلى 3 منتجات
                const additionalProducts = productsData.filter(product => !featuredOffers.some(o => o.id === product.id));
                displayOffers = [...featuredOffers, ...additionalProducts.slice(0, 3 - featuredOffers.length)];
            }

            // تفريغ الحاوية
            if (randomOffers) {
                // تغيير عنوان القسم من عروض عشوائية إلى عروض مميزة
                const sectionHeader = document.querySelector('#randomOffersSection .section-header h2');
                if (sectionHeader) {
                    sectionHeader.innerHTML = '<div class="category-icon random-icon"><i class="fas fa-star"></i></div> عروض مميزة';
                }
                ...
                // إضافة المنتجات إلى الحاوية
                displayOffers.forEach(product => {
                    randomOffers.appendChild(createProductCard(product));
                });
            }
        } catch (error) {
            console.error("حدث خطأ أثناء تحميل العروض المميزة:", error);
        }
    }
